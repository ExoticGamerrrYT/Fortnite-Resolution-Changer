...
int main(int, char**)
{
    ...
    // Alertbox text
    LPCWSTR title = L"Informer";
    LPCWSTR message = L"Done!";
    ...
    while (!done)
    {
        ...
        ImGui::Begin("Settings", NULL, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize);

        ImGui::InputInt("Width", &width, 0, 0);
        ImGui::InputInt("Height", &height, 0, 0);

        ImGui::Text("Window Mode");
        ImGui::RadioButton("Fullscreen", &window_mode, 0); ImGui::SameLine();
        ImGui::RadioButton("Windowed Fullscreen", &window_mode, 1); ImGui::SameLine();
        ImGui::RadioButton("Windowed", &window_mode, 2);

        ImGui::Checkbox("Read-only", &read_only);

        if (ImGui::Button("Apply", ImVec2(ImGui::GetContentRegionAvail().x - 15, 40)))
        {
            // Clear errors
            errorMessages.clear();
            ...
            // Chech if it is read-only and make it not read-only so we can write
            if (isReadOnly(iniPath))
            {
                makeFileNotReadOnly(iniPath);
            }

            // Set resolution and window mode
            setResolution(width, height, iniPath);
            setWindowMode(window_mode, iniPath);

            // Set read-only if wanted
            if (read_only)
            {
                makeFileReadOnly(iniPath);
            }
